    ...
    <div>
        ...
        <el-table ref="multipleTable" :data="listData" tooltip-effect="dark" style="width: 100%"
            ...
            <el-table-column label="操作" fixed="right">
                <template slot-scope="scope">
                    <el-button type="primary" @click="handleEdit(scope.$index, scope.row)" icon="el-icon-edit-outline">编
                        辑</el-button>
        ...
        <el-pagination @current-change="currentChange" @size-change="sizeChange" :page-size="pageInfo.pageSize"
            :current-page="pageInfo.pageNum" background layout="prev, sizes,pager, next"
            :page-sizes="[10, 20, 30, 40, 50, 100]" :total="total">
        </el-pagination>

        <el-drawer :title="formData.id ? '编辑' : '新增'" :visible.sync="isDrawerDialog" size="70%" direction="rtl"
                    ...
                    <el-form-item label="封面图">
                        <el-upload style="text-align: center;padding-bottom:10px;" :action="$httpUploadFile"
                            ...
                            <i v-else class="el-icon-plus"
                                style="font-size: 28px;width: 150px;height: 150px;line-height: 150px;text-align: center;border: 1px solid #ccc">
                            </i>
                        </el-upload>

                    </el-form-item>
                    <el-form-item label="浏览量">
                        <el-input v-model="formData.views" autocomplete="off"></el-input>
                    </el-form-item>
    ...
    methods: {
        ...
        submit() {
            ...
            if (!this.formData.id) {
                ...
                //更新
                this.$axios.put("/news/update", this.formData).then(res => {
                    if (res.code === 200) {
                        this.isDrawerDialog = false
                        this.$message({
                            message: res.msg,
        ...
        batchDelete(rows) {
            ...
            this.$confirm(`此操作将永久删除ID为[${this.ids}]的数据, 是否继续?`, '提示', {
                ...
                this.$axios.delete("/news/delBatch", { data: this.ids }).then(res => {
                    if (res.code === 200) {
                        this.$message({
                            type: 'success',
                            message: res.msg
